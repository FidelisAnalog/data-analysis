...
def create_animation(hours_list, matrix, freqs, window_size, window_step, output_file, frame_duration, channel, harmonic, config):
    ...
    for frame in range(n_windows):
        ...
        ax2.set_title(f"Window: T{ws['start_hours']:03d}-T{ws['end_hours']:03d} (Mean: {ws['mean']:.1f}%)")
        
        # Panel 3: Summary
        window_labels = [f"T{ws2['start_hours']:03d}-\nT{ws2['end_hours']:03d}" for ws2 in window_stats]
        ...
        for i in range(frame + 1):
            ...
            ax3.bar(i, all_means[i], width=0.6, color=color, alpha=0.7)
        
        # Draw line connecting points
        if frame > 0:
            ax3.plot(range(frame + 1), all_means[:frame + 1], 'o-', color='darkblue', linewidth=2, markersize=8)
        
        # Current marker
        ax3.plot(frame, ws['mean'], 'o', color='lime', markersize=15, markeredgecolor='black', markeredgewidth=2)
